    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-[480px]">
      <div class="px-6 py-8 -shadow sm:rounded-lg sm:px-12">
        <from class="space-y-6" @submit.prevent="onLogin">
          <div>
            ...
            <div class="mt-2">
              <Input
                ...
                :on-validate="(isError) => formDataError.email = isError"
          ...
          <div>
            <label for="password" class="block text-sm font-medium leading-6 text-green-500">senha</label>
            <div class="mt-2">
              <Input
                name="password"
                type="password"
                ...
                validate-with-on-blur
                :on-change="(value) => formData.password = value"
                :on-validate="(isError) => formDataError.password = isError"
                :validate="(value) => value?.length >= 4"
                :helper-text="'senha inválida'"
                :helper-text-class="'text-white font-semibold bg-orange-400 p-1 rounded-md'"
                :validate-on-update="isValideAll"
                autocomplete="off"
              />
          ...
          <div v-else>
            <Button type="submit" class="w-full" @click="onLogin">
              Entrar
            </Button>
          </div>
        </from>
      ...
      <p v-if="isError" class="m-auto mt-5 text-center text-md text-red-600 bg-red-200 w-fit p-5 rounded-md">
        <span href="#" class="-font-semibold leading-6">Credenciais inválidas</span>
      </p>
    </div>
  </div>
</template>

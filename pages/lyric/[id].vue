<template>
  <div class="text-center bg-black text-white w-screen h-screen">
    <div v-if="lyric" class="fixed ml-2 grid grid-rows-3">
      <NuxtLink to="/lyrics" class="col-span-1 text-center pt-6 mb-2" title="go to home">
      ...
      <NuxtLink :to="'/edit?lyric=' + lyric.id" title="Go to lyric editing" class="col-span-1 focus:outline-none focus:ring-2 focus:ring-opacity-75">
        <PencilSquareIcon class="w-6 h-6 text-green-500"/>
      </NuxtLink>

      <div v-if="isEdit" @click="onSave" role="button" class="col-span-1 focus:outline-none focus:ring-2 focus:ring-opacity-75">
        <CheckIcon class="w-6 h-6 text-green-500"/>
      </div>

      <div v-else @click="isEdit = true" role="button" :to="'/edit?lyric=' + lyric.id" title="Edit this part" class="col-span-1 focus:outline-none focus:ring-2 focus:ring-opacity-75">
    ...
    <template v-if="lyric">
      ...
      <div v-if="isEdit" class="mb-5 m-auto w-full h-auto max-w-full md:w-96 md:h-52">
        <div class="-col-span-11">
          ...
          <input
            ...
            v-model="currentStep.time"
          />
        </div>

        <div class="mb-5">
          <label for="large-input" class="text-start block mb-2 text-sm font-medium text-gray-900 dark:text-white">Part {{ $route.query.part }}</label>
          <textarea
            ...
            v-model="currentStep.html" placeholder="lyrics HTML here..."
            rows="5"
          />
        </div>
      </div>
...
import { PencilSquareIcon, CheckIcon, PencilIcon, } from "@heroicons/vue/24/outline";
